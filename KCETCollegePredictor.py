...
df = pd.read_csv("https://github.com/VishnuSastryHK/KCETCollegePredictor/blob/master/CET_Database_Final2019.csv", sep=r'\s*,\s*',names=col_names, header=None, 
                                engine="python", delimiter = "," , error_bad_lines=False)#)#,sep=r'\s*,\s*', sep="\s+|;|:", error_bad_lines=False)# names=col_names)
